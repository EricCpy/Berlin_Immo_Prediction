...
- we will do HPO only on the first gibbs otherwise it takes too long and as gibbs leaves the distributations etc same parameters are likely to still be near the optimal param
...
To evaluate these statements, we plotted a regression line for `livingSpace`, one of the variables with the highest correlation to `baseRent`. The plot shows that the model does not overfit to all outliers, but the still appears to be strongly overfitted.
