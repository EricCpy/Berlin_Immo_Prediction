...
- "@molnar_guide_explainable_models_2024"
- "@xgboost_tutorial_2022"
editor_options: 
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit}
data <- data.frame(
  Column = c("regio1", "geo_bln", "geo_krs", "regio2", "street", "telekomTvOffer", 
                  "telekomHybridUploadSpeed", "scoutId", "yearConstructedRange", "streetPlain", 
                  "houseNumber", "baseRentRange", "livingSpaceRange", "noRoomsRange", 
                  "electricityKwhPrice", "electricityBasePrice", "energyEfficiencyClass", 
                  "date", "description", "facilities", "totalRent", "newlyConst"),
  Reason_for_Removal = c("Redundant, as it contains only 'Berlin.'", "Redundant, as it contains only 'Berlin.'", 
                         "Redundant, as it contains only 'Berlin.'", "Redundant, as it contains only 'Berlin.'", 
                         "Redundant information; contains the same data as `street_plain`, but of lower quality.", 
                         "Irrelevant, as all listings can offer this service.", 
                         "Unnecessary, with mostly missing (NA) values.", "Unique ID, not useful for analysis.", 
                         "Artificial column, not relevant to the analysis.", 
                         "Would require significant cleaning or conversion into longitude and latitude.", 
                         "Requires a corresponding street to provide meaningful context.", 
                         "Artificial column, not needed for analysis.", "Artificial column, not needed for analysis.", 
                         "Artificial column, not needed for analysis.", 
                         "Deprecated since 2020; irrelevant as the dataset lacks earlier data.", 
                         "Deprecated since 2020; irrelevant.", "Deprecated since 2020; irrelevant.", 
                         "Redundant, as it duplicates `date_full`.", 
                         "Redundant, as it only repeats information already provided in other columns.", 
                         "Redundant, as it only repeats information already provided in other columns.", 
                         "Artificial column, a sum of baseRent, heatingCosts, and serviceCharge; unnecessary for analysis.", 
                         "Artificial column, constructionYear >= 2017 ? TRUE : FALSE")
)

kable(data, caption = "Columns and Reasons for Removal", align = "l", col.names = c("Column                            ", "Reason for Removal"))
```
...
The implementation of all basic models is included in `eric_model.R`. Hyperparameter optimization (HPO) is performed exclusively on a single Gibbs-imputed dataset using 10-fold cross-validation in `eric_hpo.R`. This approach was selected to save computational resources, as Gibbs iterations maintain consistent distributions and variables with the highest correlations in our dataset have fewer missing values.

All models are then trained using the hyperparameters determined through HPO with an 80/20 train-test split on all of our imputed datasets to evaluate their performance. Additionally, outlier management is an aspect of our analysis, with experiments conducted both including and excluding outliers. Outlier free datasets are reserved for a final comparison. Finally, we will explore if it is possible to reduce the model size and evaluate how the models perform with fewer columns and potentially less noise. The following table summarizes the results of this chapter:

```{r, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit}
model_comparison <- data.frame(
  Method = c("Regression Tree", "SVM", "Random Forest", "Gradient Boosting", "XGBoost"),
  `Average MSE` = c(129591.7, 114781.2, 92827.36, 92214.98, 73160.73),
  `Average RMSE` = c(359.91, 338.71, 304.65, 303.33, 270.44),
  `Average MAE` = c(207.78, 169.77, 167.32, 170.39, 154.07)
)

kable(model_comparison, caption = "Comparison of Model Performance", align = "l", col.names = c("Method", "Average MSE", "Average RMSE", "Average MAE"))
```


```{r createGibbsTrainTest, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE}
...
```{r cpTable, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit}
default_tree <- rpart(
  baseRent ~ .,
  data = train_data1,
  method = "anova"
)
cp_table <- default_tree$cptable
kable(cp_table, caption = "Complexity Table for Tree", align = "l")
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit}
model_comparison_tree <- data.frame(
  Method = c("Full Tree", "Best xError", "Best (Authors' Method)"),
  `Average MSE` = c(130515.9, 129591.7, 165022.5),
  `Average RMSE` = c(361.22, 359.91, 405.1),
  `Average MAE` = c(204.53, 207.78, 253.89)
)

kable(model_comparison_tree, caption = "Comparison of Tree Performance", align = "l", col.names = c("Method", "Average MSE", "Average RMSE", "Average MAE"))
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit}
model_svm <- data.frame(
  Method = c("SVM"),
  `Average MSE` = c(114781.2),
  `Average RMSE` = c(338.71),
  `Average MAE` = c(169.77)
)

kable(model_svm, caption = "SVM Performance", align = "l", col.names = c("Method", "Average MSE", "Average RMSE", "Average MAE"))
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit}
model_comparison_rf <- data.frame(
  Method = c("Random Forest"),
  `Average MSE` = c(92827.36),
  `Average RMSE` = c(304.65),
  `Average MAE` = c(167.32)
)

kable(model_comparison_rf, caption = "Random Forest Performance", align = "l", col.names = c("Method", "Average MSE", "Average RMSE", "Average MAE"))
...
- **Distribution**: (**gaussian**)  
  This specifies the loss function optimized by the GBM. When set to `gaussian`, the model optimizes the MSE by assuming the target variable follows a gaussian distribution.

Using the best combination (**bold**), we evaluated the GBMs performance across all imputed datasets, summarized as follows:  

```{r, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit}
model_comparison_gbm <- data.frame(
  Method = c("GBM"),
  `Average MSE` = c(92214.98),
  `Average RMSE` = c(303.33),
  `Average MAE` = c(170.39)
)

kable(model_comparison_gbm, caption = "GBM Performance", align = "l", col.names = c("Method", "Average MSE", "Average RMSE", "Average MAE"))
```

A partial dependence plot illustrates the relationship between a predictor variable and the target variable while holding other features constant. This visualization helps interpret the influence of individual predictors on the models predictions and provides insights into how a specific variable affects the target. [@molnar_guide_explainable_models_2024]  
The plot below highlights the relationship between `livingSpace` and `baseRent`.

```{r pdpGbm, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Partial Dependence Plot", fig.align='center', fig.width=5, fig.height=4}
...
```{r gbmVarImportance, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="GBM Variable Influence", fig.align='center', fig.width=5, fig.height=4}
img <- readPNG("./documents/images/gbm_variable_influence.png")
grid::grid.raster(img)
```
...
In this section, we focus on training an **XGBoost** model using the `xgboost` package in R. [@xgboost_tutorial_2022] 
XGBoost is an optimized implementation of gradient boosting that often leads to better and faster results.

Before training the model, it is essential to preprocess the categorical variables. XGBoost in R requires the input data to be in a numeric matrix form, so we need to *one-hot encode* or *label encode* the categorical variables. In our dataset none of the categorical variables have an inherent order, meaning they are nominal. Because of that we used **one-hot encoding** instead than *label encoding* to avoid introducing any unintended ordinal relationships.

To optimize the XGBoost model, we performed a *random grid search* over a range of hyperparameters. Given the computational cost of a full grid search, we implemented a random sampling strategy. The parameters explored are as follows:

- **Learning Rate (eta)**:: (0.01, 0.05, **0.1**, 0.3, 0.5)  
  The learning rate controls the contribution of each tree. A smaller eta can lead to better generalization but requires more trees to achieve convergence.
...
- **Data Sample**: (0.6, **0.8**, 1.0)  
  The proportion of rows used for training each tree. Lower values help reduce overfitting by introducing randomness, but too low may hurt performance.
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit}
model_comparison_xgb <- data.frame(
  Method = c("XGBoost"),
  `Average MSE` = c(73160.73),
  `Average RMSE` = c(270.44),
  `Average MAE` = c(154.07)
)

kable(model_comparison_xgb, caption = "XGBoost Performance", align = "l", col.names = c("Method", "Average MSE", "Average RMSE", "Average MAE"))
```

We can observe that XGBoost outperforms GBMs and to better understand why, we examined the models variable importance.
The variable importance analysis for the XGBoost model reveals slight differences compared to the GBM model. For example the `district` column holds more importance than the `regio3` column (subdistricts), suggesting that we can potentially remove `regio3` without significant loss of information.

```{r xgboostVarImportance, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="XGB Variable Influence", fig.align='center', fig.width=5, fig.height=4}
img <- readPNG("./documents/images/xgb_variable_influence.png")
grid::grid.raster(img)
```

XGBoost delivered the best results out of all methods tested, even with the default parameters. This demonstrates XGBoosts the ability to capture the most variance in the data while avoiding overfitting and XGBoosts state-of-the-art model status.



## Outlier Handling

The dataset has a significant variance in prices, with few high-priced apartments that create a *long-tail* distribution. To assess the robustness of our models against outliers, we will begin by examining the true vs. predicted values for our best performing model, which was XGBoost.

```{r xgbTruePredicted, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="True vs Predicted Base Rent", fig.align='center', fig.width=6, fig.height=4}
img <- readPNG("./documents/images/xgb_true_predicted.png")
grid::grid.raster(img)
```

The plot reveals that the model performs well for most non-outlier base rents, with predictions closely aligned to the true values. However, there is noticeable underprediction for base rents above 2,600, while higher base rents tend to have wider variation in predictions. Despite this, the model demonstrates some robustness against outliers.  
...
There are several reasons we think it might be benficial to get better predictions for more common apartments:

- Outliers may represent corrupted data that do not occur in real-world scenarios.  
- Most potential users of the model may not be wealthy and are likely more interested in predictions for mid-range apartments.  
- Prioritizing accuracy for mid-range apartments aligns better with the needs of the target audience, ensuring more practical and relevant predictions.  
...
To detect outlier apartments, we use a boxplot logic. After this definition, all values bigger or smaller than 1.5 times the interquartile range from the median get excluded. The following plot shows the `baseRent` outliers for our whole dataset:

```{r boxplotBaseRent, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Boxplot for Base Rent", fig.align='center', fig.width=5, fig.height=4}
ggplot(berlin_data_for_model, aes(x = baseRent)) + 
    geom_boxplot() + 
    labs(x = "Base Rent", y = "") +
...
We can see from the boxplot, that there are only expensive apartments that are considered outliers. Therefore it is only necessary to calculate the upper bound to define it as an outlier border.

```{r outlierBoundCalculation, include=FALSE, echo=FALSE, message=FALSE, warning=FALSE}
iqr <- IQR(berlin_data_for_model$baseRent)
q3 <- quantile(berlin_data_cleaned$baseRent, 0.75)
upper_bound <- unname(q3 + (1.5 * iqr))
```
When calculating the upper bound, all flats with a price higher than `r toString(upper_bound)`€ should be excluded.
...
It is important to note that we did not optimize the hyperparameters for the models trained on the dataset without outliers. All models were evaluated on outlier free data. The error metrics are presented in the table below:

```{r, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit}
model_comparison_outlier <- data.frame(
  Method = c("Regression Tree", "Regression Tree", "SVM", "SVM", "Random Forest", 
             "Random Forest", "Gradient Boosting", "Gradient Boosting", "XGBoost", "XGBoost"),
  `Trained With Outliers` = c("YES", "NO", "YES", "NO", "YES", "NO", "YES", "NO", "YES", "NO"),
  `Average MSE` = c(63658, 52322, 43701, 38546, 44653, 39227, 46736, 34438, 39065, 32187),
  `Average RMSE` = c(252, 229, 209, 196, 211, 198, 216, 186, 198, 179),
  `Average MAE` = c(179, 168, 143, 138, 146, 139, 154, 135, 134, 126)
)

kable(model_comparison_outlier, caption = "Comparison of Model Performance Metrics with/without Outliers", align = "l" , col.names = c("Method", "Trained With Outliers", "Average MSE", "Average RMSE", "Average MAE"))
```

Here we can see that all methods significantly improved on the outlier-free data when we trained them without outliers. 
The order of the models, based on their improvement in RMSE (italic) after removing outliers, is as follows:
