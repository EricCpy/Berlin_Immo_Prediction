...
|---------------------------|--------------|--------------|--------------|
| **Regression Tree**       | 129,591.7    | 359.91       | 207.78       |
| **SVM**                   | 114,781.2    | 338.71       | 169.77       |
| **Random Forest**         | 92,827.36    | 304.65       | 167.32       |
| **Gradient Boosting**     | 92,214.98    | 303.3272     | 170.3922     |
| **XGBoost**               | 73,160.73    | 270.4421     | 154.0709     |
...
| Method                | Average MSE  | Average RMSE | Average MAE  |
|-----------------------|--------------|--------------|--------------|
| **Gradient Boosting** | 92214.98     | 303.3272     | 170.3922     |  
...
| Method                | Average MSE  | Average RMSE | Average MAE  |
|-----------------------|--------------|--------------|--------------|
| **XGBoost**           | 73160.73     | 270.4421     | 154.0709     |
...
XGBoost delivered the best results out of all methods tested, even with the default parameters. This demonstrates XGBoosts the ability to capture the most variance in the data while avoiding overfitting and making it a state-of-the-art problem.



## Outlier Handling

The dataset has a significant variance in prices, with a few high-priced apartments in the distribution (*long-tail*). To evaluate the robustness of our models against outliers, we propose the following tests:  

1. **Training and Testing on Outlier-Free Data**:  
   We will retrain the models on a dataset that excludes outliers and test their performance on outlier-free test data.  

2. **Testing Existing Models on Outlier-Free Data**:  
   We will assess the performance of our previously trained models (which included outliers during training) on the outlier-free test data.  
...
**Why focus on more common apartments?**  
...
The order of the models, based on their improvement in RMSE (bold) after removing outliers, is as follows:
...
- TODO plot predicted values of best model against real values

## Model Size Reduction (REFACTOR)
When looking at the variable important of a random forest classifier, the variables
contributing most to node purity are livingSpace, which has by far the most impact,
followed by serviceCharge. Both of these variables have a strong positive correlation
with baseRent. There are also variables that have no significant impact at all, like
balcony, cellar, garden, telekomUploadSpeed, noParkSpaces, lift, hasKitchen, petsAllowed,
numberOfFloors, heatingType, floor, lastRefurbish, pricetrend, pictureCount, typeOfFlat
and firingTypes. For these variables, noParkSpaces, lastRefurbish and petsAllowed
have the most missing values, making them even more unattractive to use. <br>
Based on correlation we should include heatingCosts and noRooms, as they also
strongly correlate with baseRent. <br>
The variables priceTrend, telekomUplaodSpeed, yearConstructed, floor, numberofFloors and
lastRefurbish correlate very weakly with baseRent, considering the variable importance in relation
to node impurity, yearConstructed ranks pretty high, why it will be considered.
When taking the different categories of condition into account, we could show that
it has an impact on baseRent, which makes sense considering that apartments in good condition
are more attractive for rent. <br>
To see the potential impact of models, PCA could be done to see what variables contribute
a lot of variance in the data. For categorical variables, the same could be done with
MCA. <br>
Based on the discussion on the different aspects of variable important, the models
will be trained only on the following selected variables to evaluate the performance: <br>
heatingType, yearConstructed, hasKitchen, livingSpace, condition, interiorQual,
typeOfFlat, noRooms, thermalChar, heatingCosts and district.
